...
export const setRDHead = (req, res) => {
  ...
  const qur = `UPDATE bcsir.department SET RDHeadID =${ID} WHERE DepartmentID = ${req.body.dept}; `;
  const qur2 = `UPDATE bcsir.researcher SET type=RDHeadID  WHERE ID = ${ID}; `;
  console.log(qur);
  db.query(qur, function (err, result) {
    if (err) {
      console.log("Something happend for for set info");
      ...
      db.query(qur2, (err, data) => {
        if (err) {
          console.log("err set type");
          return res.status(409).json("department not found ");
        } else {

    ...
    if (result.data === "Password matches") {
      try {
        if (userType === "admin") {
          ...
          if (actionType === "add") {
            console.log(
              "Director adding researcher:",
              newResearcherEmail,
              info[0].departmentID
            );
            await axios.post(
              "http://localhost:3001/app/addResearcherByDirector",
              input
            );
            console.log("Successfully added researcher");
          } else if (actionType === "remove") {
      ...
      <div className="shade2 p-5 rounded">
        ...
        <form>
          <div className="p-2">
            ...
            <select
              value={actionType}
              onChange={handleActionTypeSelection}
              required
            >
              <option value="">Select</option>
              <option style={{ color: "black" }} value="add">
                Add Researcher
              </option>
              <option style={{ color: "black" }} value="remove">
                Remove Researcher
              </option>
            </select>{" "}
            <hr />
          ...
          {actionType === "add" && (
            <div className="p-2">
              ...
              <input
                ...
                onChange={handleNewResearcherEmailChange}
                required
              />{" "}
              <hr />
            </div>
          ...
          {userType === "admin" &&
            isActionTypeSet &&
            isDepartmentSet &&
            (isNewResearcherSet || isSelectedResearcherSet) && (
              <input
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <p>
                    ...
                    {actionType === "add" ? "add" : "remove"} a researcher?
                    {actionType === "add"
                      ? ` Email: ${newResearcherEmail}`
                      : ` Selected Researcher: ${selectedResearcher}`}
                    {userType === "admin" &&

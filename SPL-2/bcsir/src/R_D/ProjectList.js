...
const ProjectList = () => {
  ...
  useEffect(() => {
    const handleProfileClick = async () => {
      const output = await axios.post(
        "http://localhost:3001/RD/getProjectList"
      );
  ...
  const updateProgress = (projectId, newProgress) => {
    ...
    axios.post("http://localhost:3001/RD/storeListUpdate", {
            ...
            {editPermission && (
              <div>
                <input
                  ...
                  onChange={(e) =>
                    updateProgress(project.ResearchID, e.target.value)
                  }
                />
                <button
                  className="btn btn-outline-light"
                  onClick={() =>
                    updateProgress(project.ResearchID, project.Progress)
                  }
                >
                  Update Progress
                </button>
